...
def simular_partida(team_id_home, team_id_away):
    ...
    team_home_over = calc_team_overall(team_id_home)
    team_away_over = calc_team_overall(team_id_away)
        
    diff = int(((min(team_home_over, team_away_over)/2) / max(team_home_over, team_away_over))*100)
    
    if team_home_over > team_away_over:
        home_chance = 100 - diff
        away_chance = diff
    elif team_away_over > team_home_over:
        away_chance = 100 - diff
        home_chance = diff
    else:
        away_chance = diff
        home_chance = diff
